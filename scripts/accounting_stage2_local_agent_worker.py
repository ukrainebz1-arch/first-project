...
def safe_get(u,timeout=6):
    try:
        ...
        if r.status_code==200 and ('text/html' in ct or 'text/plain' in ct):
            r.encoding=r.apparent_encoding or r.encoding; return r.url,r.text[:900000]
    except:pass
    return '',''
def cleantext(html):return re.sub(r'\s+',' ',BeautifulSoup(html or '','html.parser').get_text(' ',strip=True))
def relevant_snippets(txt,limit=6):
    low=txt.lower();out=[]
    for word in EMP_WORDS:
        ...
        while len(out)<limit:
            ...
            if i<0:break
            sn=re.sub(r'\s+',' ',txt[max(0,i-150):min(len(txt),i+260)]).strip()
...
def search(q):
    for base in ['https://www.bing.com/search?q=','https://html.duckduckgo.com/html/?q=']:
        try:
            r=requests.get(base+quote_plus(q),headers=H,timeout=7)
            if r.status_code!=200:continue
            b=BeautifulSoup(r.text,'html.parser');items=[]
            if 'bing.com' in base:
                for x in b.select('li.b_algo')[:6]:
...
def collect(row):
    ...
    for site in websites[:1]:
        ...
        for uu,hh in queue:
            if uu in seen or len(pages)>=6:continue
            ...
            for href in links[:6]:
                if href not in seen and len(queue)<6:
                    u2,h2=safe_get(href)
    ...
    for it in sr[:8]:
        ...
        uu,hh=safe_get(u)
        if hh:fetched.append({'url':uu,'source_type':'search_result_page','snippets':relevant_snippets(cleantext(hh),5)})
        if len(fetched)>=2:break
    return {'official_pages':pages,'team_profile_links':len(team_profiles),'team_domain_emails':len(emails),'job_links':len(joblinks),'search_results':sr[:12],'fetched_pages':fetched}
...
def ollama_chat(base,model,prompt):
    payload={'model':model,'stream':False,'format':'json','messages':[{'role':'system','content':'Rigorous Austrian B2B company research analyst. Return only valid JSON.'},{'role':'user','content':prompt}], 'options':{'temperature':0,'num_ctx':8192,'num_predict':1000}}
    r=requests.post(base.rstrip('/')+'/api/chat',json=payload,timeout=180);r.raise_for_status();return json.loads(r.json()['message']['content'].strip())
...
def ground(obj,row,research):
    ...
    for x in obj.get('evidence') or []:
        if isinstance(x,dict) and (x.get('url') or '').strip() in allowed:
            ev.append({'url':x.get('url').strip(),'source_type':x.get('source_type','other'),'fact':str(x.get('fact',''))[:400],'supports':str(x.get('supports',''))[:400]})
    obj['evidence']=ev
...
def main():
    ...
    for i,row in enumerate(rows,1):
        ...
        try:
            ...
            obj=ground(ollama_chat(args.ollama_base,args.model,critic_prompt(first,pkt)),row,research) if first.get('verdict') in PROMOTED or first.get('verdict')=='BELOW_20' else first
