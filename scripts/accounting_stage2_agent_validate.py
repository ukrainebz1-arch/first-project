...
def validate_obj(obj, strict=True):
    ...
    verdict=obj.get('verdict'); scope=obj.get('count_scope')
    if verdict in PROMOTED:
        ...
        if scope not in {'AUSTRIA_GROUP','AUSTRIA_LEGAL_ENTITY'}: errors.append('promoted verdict cannot rely on local-office/global-network/unknown scope')
    if verdict=='BELOW_20' and good_urls < 1: errors.append('BELOW_20 requires at least one source URL')
    if strict and len((obj.get('research_summary') or '').strip()) < 20: errors.append('research_summary too short')
    return errors
...
def chunk_mode(args):
    input_rows=csv_rows(args.input_csv); keys=[r['group_key'] for r in input_rows]
    objs=load_jsonl(args.result_jsonl); errors=[]; seen=[]
    for obj in objs:
    ...
    if set(seen)!=set(keys): errors.append({'global':'group_key set mismatch','missing':sorted(set(keys)-set(seen)),'extra':sorted(set(seen)-set(keys))})
...
def merge_mode(args):
    original=csv_rows(args.original_csv); by_key={r['group_key']:r for r in original}
    candidates=csv_rows(args.candidate_csv); candidate_meta={r['group_key']:r for r in candidates}
    ...
    for obj in objs:
        ...
        if k not in candidate_keys: errors.append({'group_key':k,'errors':['not in prepared candidate universe']}); continue
    ...
    for obj in validated:
        base=dict(by_key[obj['group_key']]); cm=candidate_meta.get(obj['group_key'],{})
        ev=obj.get('evidence') or []
        base.update({
            'agent_selection_reason':cm.get('selection_reason',''),
            'agent_research_mode':cm.get('research_mode',''),
            ...
            'agent_count_scope':obj['count_scope'],'agent_confidence':obj['confidence'],
            ...
            'agent_review_note':obj.get('review_note',''),'agent_researcher_consensus':obj.get('researcher_consensus',''),
        })
        outrows.append(base)
    ...
    if outrows:
        ...
        def write(name,rows):
            with open(os.path.join(args.output_dir,name),'w',encoding='utf-8-sig',newline='') as f:
                w=csv.DictWriter(f,fieldnames=fields); w.writeheader(); w.writerows(rows)
        write('stage2_agent_verified_all.csv',outrows)
        confirmed=[r for r in outrows if r['agent_verdict'] in PROMOTED]
        write('stage3_size_verified_targets.csv',confirmed)
        recovered=[r for r in confirmed if r.get('qualification_status')=='NO_20PLUS_EVIDENCE']
        write('false_negatives_recovered.csv',recovered)
        likely=[r for r in outrows if r['agent_verdict']=='LIKELY_20_PLUS']
        write('stage2_still_likely_20plus.csv',likely)

    summary={
        'candidate_expected':len(candidate_keys),'agent_results_seen':len(seen),'agent_results_valid':len(validated),
        'missing':len(missing),'invalid':len(errors),'verdict_counts':dict(Counter(o['verdict'] for o in validated)),
        'stage3_confirmed_targets':sum(o['verdict'] in PROMOTED for o in validated),
        'recovered_false_negatives':sum(o['verdict'] in PROMOTED and by_key[o['group_key']].get('qualification_status')=='NO_20PLUS_EVIDENCE' for o in validated),
        'still_likely_20plus':sum(o['verdict']=='LIKELY_20_PLUS' for o in validated),
        'below_20':sum(o['verdict']=='BELOW_20' for o in validated),'unresolved':sum(o['verdict']=='UNRESOLVED' for o in validated),
        'missing_group_keys':missing,'errors':errors,
    }
...
def main():
    ...
    m=sub.add_parser('merge'); m.add_argument('--original-csv',required=True); m.add_argument('--candidate-csv',required=True); m.add_argument('--results-dir',required=True); m.add_argument('--output-dir',required=True)
    args=ap.parse_args(); chunk_mode(args) if args.mode=='chunk' else merge_mode(args)
